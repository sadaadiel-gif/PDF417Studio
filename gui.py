...
class PDF417Studio:
    def __init__(self):
        ...
        self._build_ui()
        threading.Thread(target=updater.check_for_update, args=(self.root,), daemon=True).start()
    ...
    def _build_main_info(self, parent):
        ...
        self.issuer_id = self.issuer_id_combo
        ...
        self.first  = _entry(g, width=14)
        self.first.grid(row=3, column=0, sticky="ew")
        self.middle = _entry(g, width=10)
        self.middle.grid(row=3, column=1, sticky="ew", padx=(6,0))
        self.last   = _entry(g, width=16)
        self.last.grid(row=3, column=2, sticky="ew", padx=(6,0))
        ...
        self.lic = _entry(g, width=14)
        self.lic.grid(row=5, column=0, sticky="ew")
        self.sex_var = tk.StringVar(value="1 – M")
        _combo(g, self.sex_var, _SEX_OPTIONS, width=8).grid(row=5, column=1, sticky="ew", padx=(6,0))
        self.cls = _entry(g, width=10)
        self.cls.grid(row=5, column=2, sticky="ew", padx=(6,0))
        ...
        self.birth = _entry(g, width=14)
        self.birth.grid(row=7, column=0, sticky="ew")
        self.exp   = _entry(g, width=14)
        self.exp.grid(row=7, column=1, sticky="ew", padx=(6,0))
        self.issue = _entry(g, width=14)
        self.issue.grid(row=7, column=2, sticky="ew", padx=(6,0))
        _lbl(g, "Address").grid(row=8, column=0, sticky="w", pady=(6,0))
        _lbl(g, "City").grid(row=8, column=2, sticky="w", padx=(6,0), pady=(6,0))
        self.address = _entry(g)
        self.address.grid(row=9, column=0, columnspan=2, sticky="ew")
        self.city    = _entry(g)
        self.city.grid(row=9, column=2, sticky="ew", padx=(6,0))
        ...
        self.zip = _entry(g, width=12)
        self.zip.grid(row=11, column=1, sticky="ew", padx=(6,0))
    ...
    def _build_optional_section(self, parent):
        ...
        donor_cb = ttk.Frame(lf)
        donor_cb.grid(row=0, column=10, sticky="w", padx=4)
    ...
    def _fmt_date(self, value: str) -> str:
        ...
        if len(clean) != 8 or not clean.isdigit():
            raise ValueError(f"Date must be 8 digits (MMDDYYYY), got: {value!r}")
        return clean
    ...
    def generate_barcode(self):
        ...
        if not (ok_w and ok_h):
            issues = []
            if not ok_w:
                issues.append(f"Width  {w_mm:.2f} mm > {AAMVA_MAX_WIDTH_MM} mm")
            if not ok_h:
                issues.append(f"Height {h_mm:.2f} mm > {AAMVA_MAX_HEIGHT_MM} mm")
    ...
    def _build_security_tab(self):
        ...
        section("Support", (
            ...
            "GitHub:  https://github.com/sadaadiel-gif/PDF417Studio"
        ), "💬")
        foot = tk.Frame(inner, bg="#0d0f1a")
        foot.pack(fill="x", pady=(8, 0))
        tk.Label(foot, text="PDF417Studio  ·  Release 1.0.0  ·  Copyright © 2026 PDF417Studio. All Rights Reserved.  ·  Governed by applicable international law.",
                 bg="#0d0f1a", fg="#7a83a6",
    ...
    def _build_mrz_tab(self):
        ...
        self.mrz_country    = _entry(g, width=6)
        self.mrz_country.grid( row=1, column=1, sticky="ew", padx=(10,0))
        self.mrz_doc_num    = _entry(g, width=12)
        self.mrz_doc_num.grid( row=1, column=2, sticky="ew", padx=(10,0))
        self.mrz_opt1       = _entry(g, width=18)
        self.mrz_opt1.grid(    row=1, column=3, sticky="ew", padx=(10,0))
        ...
        self.mrz_surname     = _entry(g, width=18)
        self.mrz_surname.grid(    row=3, column=0, sticky="ew")
        self.mrz_given       = _entry(g, width=18)
        self.mrz_given.grid(      row=3, column=1, sticky="ew", padx=(10,0))
        self.mrz_nationality = _entry(g, width=6)
        self.mrz_nationality.grid(row=3, column=2, sticky="ew", padx=(10,0))
        self.mrz_opt2        = _entry(g, width=14)
        self.mrz_opt2.grid(       row=3, column=3, sticky="ew", padx=(10,0))
        ...
        self.mrz_dob = _entry(g, width=10)
        self.mrz_dob.grid(row=5, column=0, sticky="ew")
        self.mrz_gender_var = tk.StringVar(value="M")
        _combo(g, self.mrz_gender_var, ["M","F","<"], width=4).grid(
            row=5, column=1, sticky="w", padx=(10,0))
        self.mrz_expiry = _entry(g, width=10)
        self.mrz_expiry.grid(row=5, column=2, sticky="ew", padx=(10,0))
        g.columnconfigure(0, weight=1)
        g.columnconfigure(1, weight=1)
    ...
    def _mrz_generate(self):
        sel = self._mrz_format.get()
        fmt = sel[:3].strip()
        doc_type = self._normalize_doc_type(self.mrz_doc_type_var.get())
        try:
            if fmt == "TD3":
                raw = RawTD3Document(
